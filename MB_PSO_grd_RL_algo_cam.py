...
#stack different items from samples for training
#here each sample contains a tuple (current state, current action history,
# current action, reward, next state, next action history, done (whether at terminal))
#however, for a general MDP problem, only (state, action, next state, reward, done) is enough
...
class ActorCritic:
    def __init__(self,sess):
        ...
        self.trans_learning_rate = 3e-5  #1e-4 #5e-5 #5e-5
        self.reward_learning_rate = 3e-5   #1e-4 #5e-5 #5e-5
        ...
        # get the model mapping the action sequence to total future reward at each step
        # used to apply PSO for planning
        # (not needed for DL proj)
        for i in range(self.num_step):
            init_state, init_action, return_act_input, total_r, return_model = self.create_return_model(self.num_step-i)
    ...
    # transition (dynamic) model: generally f(s_t,a_t) = s_{t+1} (what we should implement for DL proj)
    # here: f(o_1:t,a_1:t-1,a_t) = o_t+1
    ...
    # reward model: generally g(s_t,a_t) = r_t (what we should implement for DL proj)
    # here: g(o_1:t,a_1:t-1,a_t) = r_t+1
    ...
    # get the model mapping the action sequence to predicted sum of future rewards at each step
    # may not needed in DL proj
    ...
    # record the interaction data to the memmory (replay buffer)
    ...
    #train transition (dynamic) model
    def _train_trans(self, samples):
        ...
        evaluation = self.trans_model.fit([cur_states,cur_act_hists,actions],new_obs, batch_size = 32, epochs = 1)
    ...
    def _train_reward(self, samples):
        ...
        evaluation = self.reward_model.fit([cur_states, cur_act_hists, actions], rewards, batch_size = 32, epochs = 1, verbose=0)
        print(evaluation.history)
        return evaluation.history['loss']
...
#memorize previous best particle for initilization (not needed for DL proj)
global_opt_1 = []
global_opt_2 = []
global_opt_3 = []
global_opt_4 = []
...
# given a future action sequence
# predict the future rewards
# For DL proj, may need to input current state as well
# Or may not be needed because this is for heuristic search
...
# for all particles (future action sequence), compute predicted rewards
# not needed for DL proj
def f(x):
...
def PSO_grad_optimize(n_particles, dimensions,  bounds, init_pose, select_num, iteration, lr, init_state, init_act):
    #Input:
    ...
    for i in range(iteration):
        ...
        gradients = actor_critic.compute_gradient(step_num,init_act,init_state,cur_action)
...
# MPC with PSO
# take the first action from the planning result
# not needed for DL proj
def act(step_num, israndom, isdeter):
    ...
    action, good_actions = PSO_grad_optimize(n_particles=15,dimensions=dimensions,
                                             ...
                                             iteration=5,lr=1e-4,init_state=init_state,init_act=init_act)
    if isdeter:
...
# for interaction
# once determined the action, submit it to ROS for simulation and feedback
# not needed for DL proj
...
#                              entire loop                                  #
# ========================================================================= #
# this part we will use openai gym instead for interaction in DL proj
def simulation():
    global obs_list
    global act_list
    ...
    for i in range(num_trials):
        ...
        for j in range(trial_len):
            ...
            print("trial:" + str(i))
            print("step:" + str(j))
            #update obs history and action history
            #in DL proj, not needed
            ...
            # get next historys (not needed for DL proj)
            ...
            #padding (not needed for DL proj)
            ...
            # record data to the memory (replay buffer)

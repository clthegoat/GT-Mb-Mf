...
class MVE_agent():
    ...
    def __init__(self, conf):
        ...
        self.reward_model = reward_model(self.dim_state,
                                         self.dim_action).to(self.device)
        self.critic_local = critic_model(self.dim_state,
                                         self.dim_action).to(self.device)
        self.critic_target = critic_model(self.dim_state,
                                          self.dim_action).to(self.device)
        self.copy_model_over(self.critic_local, self.critic_target)
        self.actor_local = actor_model(self.dim_state,
                                       self.dim_action).to(self.device)
        self.actor_target = actor_model(self.dim_state,
                                        self.dim_action).to(self.device)
    ...
    def critic_learn(self, states_actions, rewards, next_states):
        ...
        critic_pred = torch.empty(self.batch_size,
                                  self.T + 1,
                                  dtype=torch.float)
        ...
        with torch.no_grad():
            ...
            final_critic = self.critic_target(
                torch.cat((final_states, final_actions), 1))  # Q'(s_T,a_T)
            # compute sum of discounted reward and ternimal cost for each timestep t~[-1,T-1]
            critic_target = torch.empty(self.batch_size,
                                        self.T,
                                        dtype=torch.float)

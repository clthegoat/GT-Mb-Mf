...
def lqr_controller(T, F, f, C, c, V_T, v_T):
    ...
    * horizon length T
    ...
    for t in range(T - 1, -1, -1):
        ...
        q = c[t, :, :] + np.dot(F[t, :, :].T, np.dot(V, f[t, :, :])) + np.dot(
            F[t, :, :].T, v)
        ...
        #update V
        V = Qxx + np.dot(Qxu, Kt) + np.dot(Kt.T, Qux) + np.dot(
            np.dot(Kt.T, Quu), Kt)
        v = qx + np.dot(Qxu, kt) + np.dot(Kt.T, qu) + np.dot(
            np.dot(Kt.T, Quu), kt)
...
def num_hess(net, inputs, eps=1e-3):
    ...
    h = ((fpp - fpn - fnp + fnn) / (4 * eps * eps)).view(
        nBatch, nDim, nDim, fDim).squeeze(3)
    # (N*a*a)
...
def torch_hessian(f, x):
    ...
    for n in range(batch_size):
        first_drv[n] = torch.autograd.grad(loss[n],
                                           x,
                                           create_graph=True,
                                           retain_graph=True)[0][n]
        for i in range(x_dim):
            hessian[n][i] = torch.autograd.grad(first_drv[n][i],
                                                x,
                                                create_graph=True,
                                                retain_graph=True)[0][n]
...
def forward_sim(X_0, U, f, c, v, time_step=None):
    ...
    cost = 0
    for t in range(T):
        xu = torch.from_numpy(np.concatenate([X[t, :, :],
                                              U[t, :, :]])[:, 0]).float()
    ...
    * forward simulation
    ...
    def solve_ilqr(self):
        ...
        for i in range(self.iter_num):
            ...
            #solve lqr
            self.K, self.k = lqr_controller(self.T, self.F, self.f, self.C,
                                            self.c, self.V_T, self.v_T)
            ...
            for t in range(self.T):
                dx = new_X[t, :, :] - self.X[t, :, :]
                new_U[t, :, :] = self.lr * (np.dot(self.K[t, :, :], dx) +
                                            self.k[t, :, :]) + self.U[t, :, :]
                new_U[t, :, :] = np.clip(new_U[t, :, :], self.low_U, self.up_U)
                xu = torch.from_numpy(
                    np.concatenate([new_X[t, :, :],
                                    new_U[t, :, :]])[:, 0]).float()

                #forward dyn
                new_X[t + 1, :, :] = self.dyn_f(xu).detach().numpy().reshape(
                    (self.n, 1))
                new_X[t + 1, :, :] = np.clip(new_X[t + 1, :, :], self.low_X,
                                             self.up_X)
            ...
            self.XU = np.concatenate([self.X[0:self.T, :, :], self.U[:, :, :]],
                                     axis=1)
...
def test_ilqr():
    ...
    #example usage:
    time_start = time.clock()
    ilqr_ctrl = ilqr_controller(X, U, up_X, low_X, up_U, low_U, T, dyn_f,
                                cost_f, val_f, 0.5, 10, 1, 1)
